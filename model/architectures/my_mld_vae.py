...
class MldVae(nn.Module):
    ...
    def forward(self, features: Tensor, lengths: Optional[List[int]] = None):
        ...
        z, mu, logvar = self.encode(features, lengths)
        feats_rst = self.decode(z, lengths)
        return feats_rst, z, mu, logvar
    
    def compute_loss(self, features:Tensor, lengths:List[int]):
        feats_rst, z, mu, logvar = self.forward(features, lengths)
        rec_loss = self.MSELoss(feats_rst, features)
        kl_loss = self.kl_loss(mu, logvar) * 0.0001
        return kl_loss + rec_loss, rec_loss, kl_loss
    ...
    def encode(
        ...
        mask = lengths_to_mask(lengths, device,
                               max_len=self.cfg.max_motion_len)
